...
class NSGA2Optimizer(BaseOptimizer):
    def __init__(self, config, model_wrapper, model_config, logging_util, seed):
        ...
        self.iteration = 0
        self.population_size = int(self.config.get("pop_size", 100))
    ...
    def _objective(self, trial):
        ...
        if key in self.cache:
            nsga2_scores, raw_scores = self.cache[key]
        else:
            try:
                # Raw scores from framework are (0.0=Worst, 1.0=Best)
                raw_scores = self.model_wrapper.get_score(valid_hp)
                
                # NSGA-II minimizes by default, so we invert to (0.0=Best, 1.0=Worst)
                nsga2_scores = tuple(1.0 - s for s in raw_scores)
            except Exception:
                # Crash penalty
                raw_scores = tuple(0.0 for _ in range(self.num_objectives))
                nsga2_scores = tuple(float('inf') for _ in range(self.num_objectives))
                
            self.cache[key] = (nsga2_scores, raw_scores)
    ...
    def optimize(self):
        ...
        def log_pareto_front(study, trial):
            ...
            if pareto_trials:
                ...
                min_d2h = float("inf")
                # Since t.values are inverted (0 is best), ideal is [0]
